...
const LeaderAvatar: React.FC<{
  ...
  if (!photoUrl || hasError) {
    return (
      <div className={`${className} rounded-2xl bg-slate-100 border border-slate-200 text-slate-700 flex items-center justify-center font-black text-sm sm:text-base flex-shrink-0 shadow-2xs select-none`}>
        {initials}
      </div>
    );
  }
  ...
  return (
    <div className={`${className} rounded-2xl overflow-hidden border border-slate-200/80 shadow-xs flex-shrink-0 cursor-zoom-in group/photo bg-slate-50`}>
      <img
        src={photoUrl}
        alt={name}
